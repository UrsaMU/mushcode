...
function coalesce(parts) {
  ...
  for (const p of parts) {
    if (p.type === "Literal" && out.length > 0 && out[out.length - 1].type === "Literal") {
...
function peg$parse(input, options) {
  ...
  function peg$f4(head, tail) {
    ...
    return patterns.length === 1 ? patterns[0] : node("PatternAlts", { patterns });
  }
  ...
  function peg$f8(char) {
    return node("Escape", { char });
  }
  function peg$f9(chars) {
    return node("Literal", { value: chars });
  }
  function peg$f10(head, tail) {
    const commands = [head, ...tail.map((t) => t[1])];
    return commands.length === 1 ? commands[0] : node("CommandList", { commands });
  }
  ...
  function peg$f65(operand) {
    return node("LockNot", { operand });
  }
  function peg$f66(expr) {
    return expr;
  }
  function peg$f67() {
    return node("LockMe", {});
...
function parse(text, startRule = "Start") {
  try {
    return peg$parse(text, { startRule });
  } catch (err) {
    if (err instanceof peg$SyntaxError) {
...
// src/print/lock_printer.ts
function printLock(node2) {
  return printLockNode(node2, null);
}
function printLockNode(node2, parentType) {
  switch (node2.type) {
    case "LockOr": {
      const inner = node2.operands.map((op) => printLockNode(op, "LockOr")).join("|");
      return parentType === "LockAnd" ? `(${inner})` : inner;
    }
    case "LockAnd": {
      return node2.operands.map((op) => printLockNode(op, "LockAnd")).join("&");
    }
    case "LockNot": {
      const operand = node2.operand;
    ...
    case "LockDbref":
      return node2.dbref;
    case "LockFlagCheck":
      return `flag^${node2.flag}`;
    case "LockTypeCheck":
      return `type^${node2.typeName}`;
    case "LockAttrCheck":
      return `attr^${node2.attribute}`;
    case "LockPlayerName":
      return `=${node2.name}`;
    default:
      throw new Error(`printLock: unexpected node type "${node2.type}"`);
  }
}

// src/print/printer.ts
function print(node2, opts) {
  return printNode(node2, opts);
}
function printNode(node2, opts) {
  const n = node2;
  switch (node2.type) {
    // ── Commands ──────────────────────────────────────────────────────────────
    case "CommandList": {
    ...
    case "LockAttrCheck":
    case "LockPlayerName":
      return printLock(node2);
    default:
      throw new Error(`print: unhandled node type "${node2.type}"`);
  }
}
